...
def run_benchmark(
    ...
    surface_simulation: bool = False,
    preload: bool = False,
    cycle_chunks: bool = False,
):
    lon0_expected, lat0_expected = (
        -10.128929,
        -29.721205,
    )  # values from v3 using from_netcf (so assuming A-grid!)
    ...
    for chunk in xy_chunks:
        if parcelsv4:
            ...
            ds_u = xr.open_mfdataset(filenames["U"], **fileargs)[
                ["vozocrtx"]
            ].drop_vars(["nav_lon", "nav_lat"])
            ds_v = xr.open_mfdataset(filenames["V"], **fileargs)[
                ["vomecrty"]
            ].drop_vars(["nav_lon", "nav_lat"])
            ...
            ds = (
                xr.merge([ds_u, ds_v, ds_depth, ds_mesh], compat="identical")
                .rename({"vozocrtx": "U", "vomecrty": "V"})
                .rename(
                    {
                        "glamf": "lon",
                        "gphif": "lat",
        ...
        for npart in nparts:
            if cycle_chunks:
                X, Y = np.meshgrid(
                    np.linspace(-10, 10, int(np.sqrt(npart))),
                    np.linspace(-30, -20, int(np.sqrt(npart))),
                )
                lon = X.flatten()
                lat = Y.flatten()
            else:
            ...
            pset = parcels.ParticleSet(
                fieldset=fieldset, pclass=pclass, lon=lon, lat=lat
            )

            print(
                f"Running {len(lon):_} particles on {'surface' if surface_simulation else '3D'} with parcels v{4 if parcelsv4 else 3}, chunksize {chunk} ({'preloaded' if preload else 'not preloaded'}) and {interpolator} interpolator"
            )
            ...
            if trace_memory:
                ...
                tracemalloc.stop()
                print(
                    f"Memory usage: current={current / 1e6:.0f} MB, peak={peak / 1e6:.0f} MB"
                )
            else:
            ...
            if not cycle_chunks:
                assert np.allclose(pset[0].lon, lon0_expected, atol=1e-5), (
                    f"Expected lon {lon0_expected}, got {pset[0].lon}"
                )
                assert np.allclose(pset[0].lat, lat0_expected, atol=1e-5), (
                    f"Expected lat {lat0_expected}, got {pset[0].lat}"
                )
...
def main(args=None):
    ...
    run_benchmark(
        args.Interpolator, args.memory, args.surface, args.preload, args.chunks
    )
